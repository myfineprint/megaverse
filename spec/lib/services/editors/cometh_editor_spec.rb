...
require 'spec_helper'
require_relative '../../../../lib/services/editors/cometh_editor'

RSpec.describe ComethEditor do
  let(:subject) { described_class }
  let(:base_url) { ENV.fetch('BASE_URL') }

  describe '.add' do
    before do
      stub_request(:post, "#{base_url}/comeths").with(
        body: {
          'column' => 1,
          'row' => 2,
          'direction' => 'up',
    ...
    it 'makes a POST request with correct parameters' do
      response = subject.add(1, 2, { 'direction' => 'up' })
      expect(response.code).to eq('200')
    end

    it 'does not raise NotImplementedError since api is defined' do
      expect { subject.add(1, 2, 'direction' => 'up') }.not_to raise_error
    end
  ...
  describe '.remove' do
    before do
      stub_request(:delete, "#{base_url}/comeths").with(
        body: {
          'column' => 1,
          'row' => 2,
          'candidateId' => ENV.fetch('CANDIDATE_ID')
