...
require 'spec_helper'
require_relative '../../../../lib/services/editors/polyanet_editor'

RSpec.describe PolyanetEditor do
  let(:subject) { described_class }
  let(:base_url) { ENV.fetch('BASE_URL') }

  describe '.add' do
    before do
      stub_request(:post, "#{base_url}/polyanets").with(
        body: {
          'column' => 1,
          'row' => 2,
  ...
  describe '.remove' do
    before do
      stub_request(:delete, "#{base_url}/polyanets").with(
        body: {
          'column' => 1,
          'row' => 2,
          'candidateId' => ENV.fetch('CANDIDATE_ID')
